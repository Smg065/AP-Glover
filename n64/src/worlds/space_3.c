...
void MonitorCheckpointSpace3()
{
    if(gvr_current_map == MAP_SPACE_3)
    {
        for(int i = 0; i < 4; i++)
        {
            if((u32)ap_memory.pc.worlds[AP_SPACE_L3].checkpoint_checks[i].ptr != 0)
            {
                ...
                if(object->visited == 0)
                {
                    ...
                    ap_memory.pc.worlds[AP_SPACE_L3].checkpoint_checks[i].ptr = 0;
                }
            }
        }
    }
...
//         {
//             if(ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[9].ptr != 0) // spin
//             {
//                 u32 position_addr = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[9].ptr + 0x00B8;
//                 u32 gate_ptr_offset = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[9].ptr + 0x038;
//                 u32 gate_ptr = (*(u32*)gate_ptr_offset);
//                 if(gate_ptr > 0x80000000)
//                 {
//                     u32 door_open_offset = gate_ptr + 0x0050;
//                     if(!pre3_spin_stones && ap_memory.pc.items[AP_SPACE_L3_SPIN_STONES])
//                     {
//                         (*(u16*)door_open_offset) = 0x0001;
//                         (*(u32*)position_addr) = 0xC2BE0000;
//                         pre3_spin_stones = true;
//                     }
//                     if(!ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[9].collected 
//                         && ((*(u16*)door_open_offset) == 0xFFFE || (*(u16*)door_open_offset) == 0xFF9B)) // Switch not yet collected
//                     {
//                         (*(u32*)position_addr) = 0xC2B6E2FC;
//                     }
//                     if(!ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[9].collected && (*(u16*)door_open_offset) == 0xFF9B)
//                     {
...
//             if(!pre3_garib_spawn1 && pre3_garib_timer == 0 && ap_memory.pc.items[AP_SPACE_L3_DROP_GARIBS])
//             {
//                 gvr_fn_spawn_garibgroup(6,0);
//                 gvr_fn_spawn_garibgroup(9,0);
//                 pre3_garib_spawn1 = true;
//             }
//             if(ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[4].ptr != 0) // floating platforms
//             {
//                 u32 position_addr = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[4].ptr + 0x00B8;
//                 u32 gate_ptr_offset = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[4].ptr + 0x038;
//                 u32 gate_ptr = (*(u32*)gate_ptr_offset);
//                 u32 door_open_offset = gate_ptr + 0x0050;

//                 if(!ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[4].collected 
//                     && ((*(u16*)door_open_offset) == 0xFF9B || (*(u16*)door_open_offset) == 0xFFFD)) // Switch not yet collected
//                 {
//                     if(ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] == 0)
//                     {
//                         (*(u32*)position_addr) = 0x43F70000;
//                     }
//                     else if(ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] == 1)
//                     {
//                         (*(u32*)position_addr) = 0x43DCC000;
//                     }
//                     else if(ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] == 2)
//                     {
//                         (*(u32*)position_addr) = 0x43C64000;
//                     }
//                     else if(ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] == 3)
//                     {
...
//                 }
//             }
//             if(ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[0].ptr != 0) // spin platforms
//             {
...
//                 }
//             }
//             if(ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[7].ptr != 0) //glover switch
//             {
//                 u32 position_addr = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[7].ptr + 0x00B8;
//                 u32 gate_ptr_offset = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[7].ptr + 0x038;
//                 u32 gate_ptr = (*(u32*)gate_ptr_offset);
//                 u32 door_open_offset = gate_ptr + 0x0050;
...
//                 }
//             } 
//             if(ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[1].ptr != 0) // lava pit
//             {
//                 u32 position_addr = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[1].ptr + 0x00B8;
//                 u32 frame_position_addr = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[10].ptr + 0x00B8;
//                 u32 gate_ptr_offset = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[1].ptr + 0x038;
//                 u32 gate_ptr = (*(u32*)gate_ptr_offset);
//                 if(gate_ptr > 0x80000000)
//                 {
//                     u32 door_open_offset = gate_ptr + 0x0050;
//                     if(!pre3_elevator && ap_memory.pc.items[AP_SPACE_L3_DIRT_ELEVATOR])
//                     {
//                         (*(u16*)door_open_offset) = 0x0001;
...
//                     }
//                     if(!ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[1].collected && (*(u16*)door_open_offset) == 0xFF9B)
//                     {
//                         (*(u16*)door_open_offset) = 0xFFFD;
//                     }
//                 }
...
// {
//     if(!pre3_float_platforms && ap_memory.pc.items[AP_SPACE_L3_FLOATING_PLATFORMS])
//     {
//         pre3_float_platforms = true;
//         pre3_event_timer = 0xC1;
//         return gvr_fn_activate_puzzle(0x8031E430);
//     }
//     if(!pre3_spin_platforms && ap_memory.pc.items[AP_SPACE_L3_LAVA_SPINNING] && pre3_event_timer == 0)
//     {
...
//             {
//                 u32 trigger_flag = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[i].ptr + 0x14;
//                 u32 alpha_flag = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[i].ptr + 0xA6;

//                 if(!ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[i].collected && (*(u32*)0x803249E0) == ptr)
//                 {
//                     ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[i].collected = 1;
//                     (*(u16*)alpha_flag) = 0;
//                 }
//                 else if(ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[i].collected)
//                 {
//                     (*(u16*)alpha_flag) = 0;
//                 }
//                 else
...
//         }
//         if(ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] > 0 
//             && ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] == pre3_monolith_lowered)
//         {
//             ap_memory.pc.items[AP_DPAD_DOWN] = 0;
//         }
//         if(ptr == ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[2].ptr) //Monolith A //runs second
//         {
//             if(ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] >= 1 && !pre3_monolitha && ap_memory.pc.items[AP_DPAD_DOWN])
//             {
...
//             }
//         }
//         else if(ptr == ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[3].ptr) //Monolith B //runs first
//         {
//             if(ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] >= 2 && !pre3_monolithb && ap_memory.pc.items[AP_DPAD_DOWN])
//             {
//                 pre3_monolith_timer = 0xC1;
//                 pre3_monolithb = true;
//                 pre3_monolith_lowered++;
...
//             }
//         }
//         else if(ptr == ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[5].ptr) //Monolith C // runs last
//         {
//             if(ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] >= 3 && !pre3_monolithc && ap_memory.pc.items[AP_DPAD_DOWN])
//             {
//                 pre3_monolith_timer = 0xC1;
//                 pre3_monolithc = true;
//                 pre3_monolith_lowered++;
...
//             }
//         }
//         else if(ptr == ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[6].ptr && ap_memory.pc.items[AP_DPAD_DOWN]) //Monolith D // runs third
//         {
//             if(ap_memory.pc.items[AP_SPACE_L3_PROGRESSIVE_LOWER_MONOLITH] == 4 && !pre3_monolithd)
//             {
//                 pre3_monolithd = true;
//                 pre3_monolith_timer = 0xC1;
//                 pre3_monolith_lowered++;
...
//         for(int i = 0; i < 10; i++)
//         {
//             if(ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[i].ptr != 0)
//             {
//                 if(i == 2 || i == 3 || i == 5 || i == 6)
//                 {
//                     u32 trigger_flag = ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[i].ptr + 0x14;
//                     if(!ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[i].collected)
//                     {
//                         (*(u32*)trigger_flag) = 0;
//                     }
//                 }
...
// {
// 	ap_memory.pc.worlds[AP_SPACE_L3].switch_checks[8].collected = 1;
//     return;
// }
